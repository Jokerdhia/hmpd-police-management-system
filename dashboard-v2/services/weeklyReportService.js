...
function keyFor(d=new Date()){
  const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Brussels',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(d);
  const get=t=>parts.find(p=>p.type===t)?.value;
  return `${get('year')}-${get('month')}-${get('day')}`;
}
function format(r){const h=s=>`${Math.floor(Number(s||0)/3600)}h${String(Math.floor(Number(s||0)%3600/60)).padStart(2,'0')}`;const inactiveLabel=d=>Number(d)>=999?'aucun service enregistré':`${Number(d||0)}j`;return [`📊 **Rapport hebdomadaire HMPD**`,`Effectif: **${r.summary.officers}** · Score moyen actuel: **${r.summary.averageScore}/100**`,`Promotions suggérées: **${r.summary.promotionEligible}** · Inactifs 7j+: **${r.summary.inactive7}**`,``,`**Top présence — semaine précédente (7 jours)**`,...(r.topAttendance.length?r.topAttendance.map((o,i)=>`${i+1}. ${o.display_name||o.user_id} — ${h(o.week_seconds)}`):['Aucune donnée']),``,`**Promotions suggérées**`,...(r.promotions.length?r.promotions.slice(0,10).map(o=>`• ${o.display_name||o.user_id} → ${o.next_grade}`):['Aucune']),``,`**Inactifs**`,...(r.inactive.length?r.inactive.slice(0,10).map(o=>`• ${o.display_name||o.user_id} — ${inactiveLabel(o.inactive_days)}`):['Aucun'])].join('\n').slice(0,1900)}
