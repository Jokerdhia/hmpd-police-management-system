(()=>{
  ...
  async function openPromotion(id){
    try{
      ...
      const componentCards=`<div class="career-components"><div><span>⭐ Points</span><strong>${c.points?.done||0}/${c.points?.total||0}</strong><small>${c.points?.ok?'Seuil atteint':`${c.points?.remaining||0} restant(s)`}</small></div><div><span>📅 Présence</span><strong>${c.presence?.done||0}/${c.presence?.total||0}</strong></div><div><span>📋 Critères</span><strong>${c.criteria?.done||0}/${c.criteria?.total||0}</strong></div><div><span>🛡️ Discipline</span><strong>${c.discipline?.ok?'RAS':'Gelée'}</strong></div></div>`;
      ...
      const personalStats=`<section class="promo-section"><div class="promo-section-head"><div><h3>📊 Statistiques personnelles</h3><p>Activité récente du policier.</p></div></div><div class="career-components"><div><span>📅 Jours qualifiés</span><strong>${Math.min(Number(progress.daysInRank||0),Number(progress.minDays||0))}/${progress.minDays||0}</strong></div><div><span>Cette semaine</span><strong>${dur(p.serviceStats?.week_seconds||0)}</strong></div><div><span>Ce mois</span><strong>${dur(p.serviceStats?.month_seconds||0)}</strong></div><div><span>Jours validés</span><strong>${Math.min(Number(progress.daysInRank||0),Number(progress.minDays||0))}/${progress.minDays||0}</strong></div></div></section>`;
