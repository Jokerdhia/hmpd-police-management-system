(()=>{
  ...
  async function openPromotion(id){
    try{
      ...
      const personalStats=`<section class="promo-section"><div class="promo-section-head"><div><h3>📊 Statistiques personnelles</h3><p>Activité récente du policier.</p></div></div><div class="career-components"><div><span>📅 Jours qualifiés</span><strong>${progress.daysInRank||0}/${progress.minDays||0}</strong></div><div><span>Cette semaine</span><strong>${dur(p.serviceStats?.week_seconds||0)}</strong></div><div><span>Ce mois</span><strong>${dur(p.serviceStats?.month_seconds||0)}</strong></div><div><span>Jours validés</span><strong>${progress.daysInRank||0}/${progress.minDays||0}</strong></div></div></section>`;
      ...
      const hierarchyNotice=!canModifyTarget?`<div class="permission-note">🔒 Lecture seule : ${esc(p.target_access?.targetGrade||p.case.from_grade)} est supérieur à ton grade ${esc(p.target_access?.actorGrade||'non défini')}. Aucune modification n’est autorisée.</div>`:'';
