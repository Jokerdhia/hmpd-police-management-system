(()=>{
  ...
  function careerCard(p){
    const m=statusMeta(p.status),pct=Number(p.progress?.percent||0),done=Number(p.progress?.completed||0),total=Number(p.progress?.total||0),days=Math.min(Number(p.progress?.daysInRank||0),Number(p.progress?.minDays||0)),sanctions=Number(p.progress?.activeSanctions||0);
